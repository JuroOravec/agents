...
2. **Preserve conversation_id and skill_id:** Both must remain in the agent's context window for the remainder of the workflow. Add a rule/skill note: _"If context gets summarized, ensure conversation_id and skill_id are preserved—they are required for skill-eval calls."_
...
- Expected phase count comes from parsing `SKILL.md` files (same logic as `src/engine/validate/skill-phases.ts`).
...
| #                                                     | Item                                                                                                                      | Status   |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | -------- |
| [#9](https://github.com/JuroOravec/agents/issues/9)   | **Phase format enforcement** — `src/engine/validate/skill-phases.ts`, meta-skill-create, CI                                  | Done     |
